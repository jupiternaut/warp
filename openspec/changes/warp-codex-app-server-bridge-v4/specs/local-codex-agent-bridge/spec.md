...
When local Codex mode is enabled, Warp Agent requests SHALL be routed to a local Codex stream source before any Warp server-backed AI request is made. The stream source SHALL support both the V3 `exec` runner and the V4 safe renderable-event translator boundary for future app-server transport.
...
#### Scenario: Safe app-server-shaped event
- **WHEN** a local Codex app-server-shaped event is safe to render without side effects
- **THEN** Warp translates it through the safe renderable-event translator rather than through Warp AI endpoints
...
- **WHEN** Codex emits an event that the V4 translator cannot render as a native Warp card
...
- **WHEN** V4 runs with the default local Codex runner
...
- **WHEN** app-server transport mapping is incomplete
